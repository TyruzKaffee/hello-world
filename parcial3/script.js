...
function generatePyramid() {
  const height = prompt('Altura de la pirámide (1-49):');
  if (!height || height < 1 || height > 49) {
    alert('Altura inválida. Debe estar entre 1 y 49.');
    return;
  }
...
const resultDiv = document.createElement('div');
  ...
  // Sumar los números del camino y mostrar el resultado
  const sum = maxPath.reduce((accumulator, [i, j]) => accumulator + pyramid[i][j], 0);
  const sumDiv = document.createElement('div');
  sumDiv.textContent = `Resultado: ${sum}`;
  sumDiv.classList.add('sumBox');
  resultDiv.appendChild(sumDiv);
}


function resetPyramid() {
  pyramidContainer.innerHTML = ''; // Limpiar el contenido actual
}
